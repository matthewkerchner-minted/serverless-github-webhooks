...
updateInQA = async (prefix='') => {
    let jql = `status = "In QA" AND fixVersion in (release-${LAST_WEEK}, release-${THIS_WEEK}) ORDER BY key DESC`;
...
updateAllBugs = async (prefix='') => {
    let jql = `type = Bug AND (affectedVersion in (release-${LAST_WEEK}, release-${THIS_WEEK}) ` +
              `OR fixVersion in (release-${LAST_WEEK}, release-${THIS_WEEK})) ORDER BY key ASC, issuetype DESC`;
